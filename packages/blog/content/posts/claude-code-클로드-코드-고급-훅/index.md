...
여기서 **슬랙 웹훅 (Incoming Webhook)** 은 *외부에서 슬랙으로 메시지를 보내게* 해주는 통로다. [훅 이벤트](#훅hooks이란-무엇인가)가 발생하는 순간, 이 웹훅으로 메시지를 쏘면 **내
휴대폰
** 으로 알림이 떨어진다.
...
이렇게 *이벤트 (Notification·Stop)* 와 *환경 변수 관리* 까지 **컨텍스트를 명확히** 주면, 클로드가 ① 환경 변수 로딩, ② 훅 스크립트 생성, ③ `settings.json` 이벤트 설정을
한 번에 잡아준다.
