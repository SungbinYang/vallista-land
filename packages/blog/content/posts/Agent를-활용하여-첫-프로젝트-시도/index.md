...
이렇게 여러 hook들을 조합해서 구성한 Claude Code 환경을 하네스 (Harness)라고 부르기도 한다. hook 하나만 잘 설정해도 에이전트를 기다리는 시간을 크게 줄일 수 있으니, 각자 자신만의
hook을 만들어보는 것을 추천한다.
...
그런데 왜 굳이 외부 API를 연동해야 할까? 이유는 간단하다. Claude Code에 붙어 있는 Opus 모델의 API가 너무 비싸기 때문이다. 실제로 에이전틱 코딩 좀 한다 하는 사람들은 대부분 GLM-5 모델을
사용하고 있다고 한다. GLM-5는 중국의 AI 기업 Zhipu AI가 개발하고 Z.AI 플랫폼을 통해 서비스되는 대형 언어 모델로, 코딩과 AI Agent 작업에 특화되어 있으며 오픈소스 모델 중에서는 최고 수준의
코딩 성능을 자랑한다. 실제 코딩 능력을 측정하는 벤치마크인 SWE-bench Verified에서 77.8%를 기록하며 Claude Opus 4.5와 유사한 수준을 보여줄 정도다. 그러면서도 가격은 훨씬 저렴하다.
...
7분의 1 가격이니 상당히 경제적이다. 참고로 GLM-4까지는 반값 혜택이 있었는데 GLM-5부터는 사라졌다고 한다. 실제로 최초 가격 대비 4배가 올랐고 앞으로 더 오를 것이라고 하니, 사용할 생각이 있다면 빨리
구독하는 것이 유리할 수 있다. 그래도 할인 혜택은 남아있다. 1년 구독 시 30% 추가 할인이 적용되고 (Max 기준 1년에 600달러 수준), z.ai 코딩플랜에 처음 가입할 경우 레퍼럴 코드를 통해 10%
할인을 받을 수 있으니 참고하자.
...
일을 시킬 수 있는 기반이 된다. 이제 이 모든 것을 합쳐서 에이전트로 첫 프로젝트를 만들어볼 차례이다. 이번에 시도해볼 것은 바로 **파워포인트 슬라이드를 자동으로 생성하는 것**이다. 그리고 이번에는 Claude
...
Claude Sonnet 계열의 바로 뒤를 잇는 수준을 보여줬고, 특히 에이전틱 (Agentic) 작업이 아닌 순수 생성 부문 즉 코드, 웹사이트, 3D 디자인 같은 영역에서는 Opus나 Fable 같은 상위
모델들을 제치고 벤치마크 1위를 기록했다. 우리가 지금 하려는 "슬라이드 (문서) 디자인 생성" 같은 작업이 바로 이 순수 생성 부문에 해당하기 때문에, GLM 5.2는 이번 실습에 아주 잘 어울리는 선택이다.
...
선택한다. High variant는 좀 더 깊게 사고 (reasoning)하는 버전이라고 이해하면 된다. 여기서도 API 키는 절대 코드나 캡처에 노출되지 않도록 주의하자.
...
직접 설치하도록 시킬 수 있어서 매우 편리하다. 이렇게 잘 만들어진 스킬 하나를 얹어주는 것만으로 결과물의 완성도가 크게 달라진다.

### 슬라이드 생성 기획하고 실행하기

준비가 끝났으면 이제 실제로 슬라이드를 만들어보자. 곧바로 "만들어줘"라고 던지기보다, 먼저 최신 문서를 참고해서 슬라이드 생성을 **기획**하도록 시키는 것이 좋다. 예를 들어 "OpenCode 사용법을 설명하는
슬라이드"처럼 주제를 정해서 요청하는 식이다. 이때 곧바로 파일을 건드리지 않고 계획부터 세우는 **플랜 (Plan) 모드**를 활용하면, 에이전트가 어떤 구조로 슬라이드를 만들지 먼저 보여준다. 진행 과정에서
테마를 고르라는 등의 선택지가 나오는데, 적당한 테마 3개 정도를 골라서 제작을 요청하면 된다.
...
그런데 처음 나온 결과물이 완벽하지는 않다. 실제로 첫 파워포인트 파일을 열어보면 **도형이 서로 겹치거나, 일부 요소가 슬라이드 캔버스 바깥으로 삐져나가는** 등의 디자인 문제가 발견된다. 여기서 "역시 AI는 안
되네"라고 포기하면 안 된다. 에이전트와의 협업에서 진짜 중요한 것은 바로 이 **검토 → 피드백 → 재요청** 루프이기 때문이다.

발견한 문제를 두루뭉술하게 말하지 말고 "도형이 겹치는 부분과 캔버스를 벗어난 요소를 수정해줘"처럼 **구체적으로** 짚어서 다시 요청한다. 그러면 에이전트가 파워포인트 파일을 다시 읽어 검증하고 문제를 고쳐준다.
이렇게 단 두 번의 프롬프트만으로도 처음 결과물과는 비교가 안 될 만큼 정돈된 슬라이드를 얻을 수 있다. 첫 결과물치고는 놀라울 정도의 완성도다.

### 첫 프로젝트를 마치며

이번 실습의 핵심은 "GLM으로 파워포인트를 만들었다"는 결과물 자체가 아니다. 앞에서 하나씩 배운 조각들, 즉 터미널 다루기, JSON 설정, 외부 API 연동, 그리고 여기에 오픈소스 스킬과 검토·피드백 루프까지
더해져서 **에이전트로 실제 산출물을 만들어내는 하나의 워크플로우**가 완성되었다는 점이 핵심이다. 그리고 이 모든 과정을 Claude 구독료보다 훨씬 저렴한 GLM 코딩플랜으로 해냈다는 것도 의미가 크다.
(z.ai 코딩플랜은 앞서 언급했듯 최초 가입 시 레퍼럴 코드로 10% 할인을 받을 수 있으니 참고하자.)

에이전트는 한 번에 완벽한 결과를 내주는 마법이 아니라, 내가 방향을 잡아주고 피드백을 주며 함께 다듬어가는 협업 파트너에 가깝다. 파워포인트가 됐든 간단한 웹앱이 됐든, 이렇게 작은 프로젝트를 직접 한 번 끝까지
만들어보는 경험이 앞으로의 에이전틱 코딩에 가장 든든한 밑거름이 될 것이다.