...
알아야 한다. 여기서 POSIX(Portable Operating System Interface)라는 개념이 등장한다. POSIX는 UNIX 계열 운영체제가 지켜야 할 표준 규격을
정의한 것으로, macOS 역시 이 POSIX 표준을 따른다. 다만 정확히 말하면 macOS는 UNIX 그 자체가 아니라, UNIX에서 갈라져 나와 애플이 커스터마이즈한
UNIX 계열(Unix-like) 운영체제다. 사용자는 운영체제에 명령을 내리거나, 입력한 명령에 대한 응답을 받을 때 터미널이라는 것을 사용한다. 터미널을 셸(shell)이라고 부르는 경우도 있다. 셸은
