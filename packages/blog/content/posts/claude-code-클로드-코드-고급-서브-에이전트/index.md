...
*인테리어* 를 한 사람이 *전 공정* 떠맡으면 완성도가 떨어지듯, 클로드에게 *이것저것 한꺼번에* 시키면 **[컨텍스트가 꼬여](/claude-code-슬래시-명령어와-단축키)** 결과가 흐려진다. 이
문제를 푸는 게 **서브 에이전트** 다.
...
*현재
프로젝트* 용과 *모든 프로젝트(전역)* 용을 고를 수 있다. 생성 과정에서 정하는 것들은 이렇다.
